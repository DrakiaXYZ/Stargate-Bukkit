...
public class Portal {
    ...
    public Location teleport(Vehicle vehicle, Portal origin) {
        Location traveller = new Location(this.world, vehicle.getLocation().getX(), vehicle.getLocation().getY(), vehicle.getLocation().getZ());
        Location exit = getExit(traveller, origin);
        double motX = vehicle.getVelocity().getX();
        double motY = vehicle.getVelocity().getY();
        double motZ = vehicle.getVelocity().getZ();
        ...
        return new Location(this.world, (motX * zz + motZ * xz) * 1, motY, (motX * zx + motZ * xx) * -1);
    }

    public Location getExit(Location traveller, Portal origin) {
        Block entrance = world.getBlockAt((int)Math.floor(traveller.getX()), (int)Math.floor(traveller.getY()), (int)Math.floor(traveller.getZ()));
        ...
        if (originExits.containsKey(entrance)) {
            ...
            if (exit != null) {
                Location loc = new Blox(exit).modRelativeLoc(0D, 0D, 1D, traveller.getYaw(), traveller.getPitch(), modX, 1, modZ);
            ...
            try {
                ...
                while (scanner.hasNextLine()) {
                    ...
                    if (!(s.getBlock().getState() instanceof Sign)) {
                    	Stargate.log.info("[Stargate] Invalid sign on line " + l);
                    	continue;
                    }
